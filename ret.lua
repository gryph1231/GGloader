...
if S.WakeofAshes:IsReady(6) and Player:HolyPower() <= 2 then
       return S.WakeofAshes:Cast()
    end

 if S.DivineStorm:IsReady(10) then
...
local function Finishers()
    ...
    varDSCastable = Cache.EnemiesCount[10] >= 3 or S.DivineJudgement:IsAvailable() and Cache.EnemiesCount[10] >= 2
    ...
    if (S.Inquisition:IsAvailable() and S.Inquisition:IsReady()) and 
			((not Player:Buff(S.Inquisition) and Player:HolyPower() >=2) or 
			(Player:BuffRemains(S.Inquisition) < 5 and Player:HolyPower() >= 3) or 
			(S.ExecutionSentence:IsAvailable() and S.ExecutionSentence:CooldownRemains() < 10 and Player.BuffRemains(S.Inquisition) < 15) or 
			(S.AvengingWrath:CooldownRemains() < 15 and Player.BuffRemains(S.Inquisition) < 20 and Player:HolyPower() >= 3)) then
        return S.INQSN:Cast()
    end
    ...
    if S.ExecutionSentence:IsReady("Melee") and (Target:IsInRange(20) and Cache.EnemiesCount[10] <= 3 and ((not S.Crusade:IsAvailable() or S.Crusade:CooldownRemains() > Player:GCD() * 2) or not RubimRH.CDsON())) then
    ...
    if S.DivineStorm:IsReady(10) and Cache.EnemiesCount[10] >= 2  then
	   return S.DivineStorm:Cast()
    end
...
local function Generators()
	...
	if S.DivineShield:IsReady() and Player:HealthPercentage()<= 20 and not Player:Debuff(S.Forbearance) then
		return S.DS:Cast()
	end

	if S.LayOnHands:IsReady() and Player:HealthPercentage()<= 20 and S.DivineShield:CooldownRemains() > 1  and not Player:Debuff(S.Forbearance) then
		return S.LoH:Cast()
	end

	if S.ShieldofVengeance:IsReady() and Player:HealthPercentage()<= 70  and not Player:Buff(S.DivineShield) then
		return S.SoV:Cast()
	end
	
	if S.BlessingofProtection:IsReady() and Player:HealthPercentage()<= 40  and not Player:Debuff(S.Forbearance) and S.DivineShield:CooldownRemains() > 1 and S.LayOnHands:CooldownRemains() > 1 then
		return S.BoP:Cast()
	end

	
	--actions.generators+ = /wake_of_ashes, if= (holy_power<=0|holy_power = 1&cooldown.blade_of_justice.remains>gcd)
    if S.WakeofAshes:IsReady() and (Player:HolyPower() <= 0 or Player:HolyPower() == 1 and S.BladeofJustice:CooldownRemains() > Player:GCD()) and Cache.EnemiesCount[10]>=3 then
        return S.WakeofAshes:Cast()
    end
	
	  if (S.Inquisition:IsAvailable() and S.Inquisition:IsReady()) and not Player:Buff(S.Inquisition) and Player:HolyPower() >=2 then
        return S.INQSN:Cast()
    end
	...
	if S.ArcaneTorrent:IsReady(6) and Player:HolyPower() <= 4 and S.CrusaderStrike:ChargesFractional() < 1 then
...
local function APL()
    ...
    if not Player:AffectingCombat() then
        ...
        -- Regular
        if S.FlashofLight:IsReady() and Player:HealthPercentage() <= 50 and Player:BuffStack(S.SelflessHealer) == 4 then
...
--	if Cache.EnemiesCount[10] >= 3 and Rubim.useAoE and AOE() ~= nil then
	--	return AOE()
--	end
